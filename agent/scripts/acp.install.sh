...
# Pure-shell YAML parsing uses sed -i per AST node; extremely slow on Windows Git Bash.
_acp_is_windows_shell() {
...
_acp_copy_all_scripts() {
    find "$TEMP_DIR/agent/scripts" -maxdepth 1 -name "*.sh" ! -name "acp.install.sh" \
        -exec cp {} "$TARGET_DIR/agent/scripts/" \;
    chmod +x "$TARGET_DIR/agent/scripts"/*.sh 2>/dev/null || true
    local _script_count
    _script_count=$(find "$TARGET_DIR/agent/scripts" -maxdepth 1 -name "*.sh" 2>/dev/null | wc -l | tr -d ' ')
    echo "${GREEN}✓${NC} Installed ${_script_count} script(s)"
}
...
if [ -d "$TEMP_DIR/agent/scripts" ]; then
    ...
    if [ -f "$TEMP_DIR/package.yaml" ]; then
        if _acp_is_windows_shell; then
            echo "Windows shell detected — copying all scripts (skipping package.yaml resolution)..."
            _acp_copy_all_scripts
        else
...
# Generate Cursor slash commands/skills after commands and scripts are installed
